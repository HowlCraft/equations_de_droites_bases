...
TESettingsFromTracks.prototype = {
	ready: function (ctrl) {
		...
		if (altVideoTrack) {
			...
			var altVideoInput = altVideoItem.appendChild(this.createInput(
				...
				'checkbox',
					"Alternate video",
					"View the alternate video",
					"Quit the alternate video")
			);
		...
		if (!ctrl.media.renderer || ctrl.media.renderer.getPlaybackRate() !== null) {
			...
			var playbackRatesBtn = playbackRatesItem.appendChild(this.createPressButton(
				...
				"Playback speed",
				"Show different playback speeds",
				"Hide different playback speeds")
			);
	...
	createPanel: function (btn, className, content) {
		...
		var focusRingBtn = panel.appendChild(this.createButton(
			this.classPrefix + 'VisuallyHidden',
			"Return",
			"Return to the start of the window", true)
		);
	...
	createSubtitleItem: function (track) {
		...
		var title = track ?  "Activate sub-titles " + track.getAttribute('srclang') : "Deactivate the subtitles";
		var label = track ? track.getAttribute('srclang') : "Deactivated";
	...
	createPlaybackRateItem: function (playbackRate) {
		var self = this;
		var item = document.createElement('li');
		let label = playbackRate == 1 ? 'Normal' : playbackRate.toString();
